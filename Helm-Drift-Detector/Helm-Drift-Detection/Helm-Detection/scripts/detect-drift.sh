...
# USAGE: ./detect-drift.sh desired.yaml
  ...
  LOG_FILE="reports/drift_report_history.csv"
  
  # Create the log file if it doesn't exist
  if [ ! -f "$LOG_FILE" ]; then
    echo "timestamp, HPA_NAME, min_replicas, max_replicas, cpu_target, svc_name, port, target_port, drift_detected" > "$LOG_FILE"
  fi
  ...
  if [[ "$drift_detected" == false ]]; then
    ...
    echo "$DATE,$LOCAL_HPA_NAME,$LOCAL_MIN_REPLICAS,$LOCAL_MAX_REPLICAS,$LOCAL_CPU_TARGET,$LOCAL_SVC_NAME,$LOCAL_PORT,$LOCAL_TARGET_PORT,No" >> "$LOG_FILE"
    exit 0
  else
    echo -e " \n Drift detected. Please review the differences."
    echo "$DATE,$LOCAL_HPA_NAME,$LOCAL_MIN_REPLICAS,$LOCAL_MAX_REPLICAS,$LOCAL_CPU_TARGET,$LOCAL_SVC_NAME,$LOCAL_PORT,$LOCAL_TARGET_PORT,Yes" >> "$LOG_FILE"
    exit 2
  fi
