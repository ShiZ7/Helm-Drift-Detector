...
# Create a CSV log file with the current timestamp
log_file="drift_report_$(date +%Y%m%dT%H%M%SZ).csv"

# Write the header for CSV file
echo "Timestamp, Drift Status, Detail" > $log_file
...
if [[ "$LOCAL_MIN_REPLICAS" != "$LIVE_MIN_REPLICAS" ]]; then
  ...
  drift_detected=true
  echo "$(date +%Y-%m-%dT%H:%M:%SZ), FAIL, minReplicas drift (Local=$LOCAL_MIN_REPLICAS, Live=$LIVE_MIN_REPLICAS)" >> $log_file
fi

if [[ "$LOCAL_MAX_REPLICAS" != "$LIVE_MAX_REPLICAS" ]]; then
  echo -e "DRIFT:\n maxReplicas (Local=$LOCAL_MAX_REPLICAS, Live=$LIVE_MAX_REPLICAS)\n"
  drift_detected=true
  echo "$(date +%Y-%m-%dT%H:%M:%SZ), FAIL, maxReplicas drift (Local=$LOCAL_MAX_REPLICAS, Live=$LIVE_MAX_REPLICAS)" >> $log_file
fi

if [[ "$LOCAL_CPU_TARGET" != "$LIVE_CPU_TARGET" ]]; then
  echo -e "DRIFT:\n CPU Target (Local=$LOCAL_CPU_TARGET%, Live=$LIVE_CPU_TARGET%)\n"
  drift_detected=true
  echo "$(date +%Y-%m-%dT%H:%M:%SZ), FAIL, CPU Target drift (Local=$LOCAL_CPU_TARGET%, Live=$LIVE_CPU_TARGET%)" >> $log_file
fi
...
if [[ "$LOCAL_PORT" != "$LIVE_PORT" ]]; then
  ...
  drift_detected=true
  echo "$(date +%Y-%m-%dT%H:%M:%SZ), FAIL, Service Port drift (Local=$LOCAL_PORT, Live=$LIVE_PORT)" >> $log_file
fi

if [[ "$LOCAL_TARGET_PORT" != "$LIVE_TARGET_PORT" ]]; then
  echo -e "DRIFT:\n Service TargetPort (Local=$LOCAL_TARGET_PORT, Live=$LIVE_TARGET_PORT)\n"
  drift_detected=true
  echo "$(date +%Y-%m-%dT%H:%M:%SZ), FAIL, Service TargetPort drift (Local=$LOCAL_TARGET_PORT, Live=$LIVE_TARGET_PORT)" >> $log_file
fi

# Final Result
if [[ "$drift_detected" == false ]]; then
  echo -e "\nSUCCESS: No drift detected."
  echo "$(date +%Y-%m-%dT%H:%M:%SZ), PASS, No drift detected" >> $log_file
  exit 0
else
  echo -e "\nDrift detected. Please review the differences."
  echo "$(date +%Y-%m-%dT%H:%M:%SZ), FAIL, Drift detected" >> $log_file
  exit 2
fi

# Copy the CSV report into the reports folder (assumes the folder exists)
cp $log_file $WORKDIR/reports/
